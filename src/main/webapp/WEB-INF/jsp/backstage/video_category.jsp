...
<div class="layui-form" lay-filter="layuiadmin-form-useradmin" id="addForm" style="padding: 10px 0 0 0;display: none">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text" name="sort" lay-verify="required" placeholder="数字越小,排序越前" autocomplete="off"
                   class="layui-input">
        </div>
    </div>
...
<div class="layui-form" id="updateForm" style="padding: 10px 0 0 0;display: none">
    ...
    <div class="layui-form-item">
        <label class="layui-form-label">分类名称</label>
        <div class="layui-input-inline">
            <input type="text" name="name" id="name" lay-verify="required" placeholder="请输入视频分类名称" autocomplete="off"
                   class="layui-input">
        </div>
    </div>
    ...
    <div class="layui-form-item">
        <label class="layui-form-label">排序</label>
        <div class="layui-input-inline">
            <input type="text" name="sort" id="sort" lay-verify="required" placeholder="数字越小,排序越前" autocomplete="off"
                   class="layui-input">
        </div>
    </div>
    ...
    layui.config({
        ...
        table.render({
            ...
            cols: [[
                ...
                {field: 'name', title: '视频分类名称', minWidth: 200},
        ...
        $('#deletes').on('click', function () {
            ...
            layer.confirm('确定要删除信息', function (index) {
                ...
                $.ajax({
                    url: '/video/category/deletes/' + ids,
                    type: "get",
                    success: function (result) {
                        if (result.code == 200) {
        ...
        table.on('tool(table)', function (obj) {
            ...
            if (layEvent === 'del') {  //删除
                layer.confirm('真的删除行么', function (index) {
                    //向服务端发送删除指令
                    $.ajax({
                        url: '/video/category/deletes/' + data.id,
                        type: "get",
                        success: function (result) {
                            if (result.code == 200) {
                                layer.msg('已删除');
