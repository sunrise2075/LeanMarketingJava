...
<div class="layui-form" lay-filter="layuiadmin-form-useradmin" id="addForm"
    ...
    <div class="layui-form-item">
        <label class="layui-form-label myLabel">视频分类名称</label>
    ...
    <div class="layui-form-item">
        <label class="layui-form-label myLabel">视频名称</label>
        <div class="layui-input-inline">
    ...
    <div class="layui-form-item">
        ...
        <span style="color: red">请上传格式为 750*350  的图片</span>
...
<script>
    layui.config({
        ...
        table.render({
            elem: '#table',
            url: '/banner/list?type=2',
            method: "get",
            cols: [[
                {type: 'checkbox', fixed: 'left', align: 'center'},
                {field: 'id', title: 'ID'},
                {field: 'title', title: '轮播图标题', minWidth: 200},
                {field: 'typeName', title: '视频名称', minWidth: 200},
                {
        ...
        form.on('select(categoryId1)', function (data) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    data = result.data;
                    var html = '<option value="">请选择视频标题</option>';
                    $.each(data, function (index, value) {
        ...
        table.on('tool(table)', function (obj) {
            ...
            if (layEvent === 'del') {  //删除
                ...
                $.ajax({
                    url: '/video/detail?id=' + data.typeId,
                    type: "get",
                    success: function (result) {
                        var videoCategory = result.data.videoCategory;
                        var video = result.data.video;
                        //赋值
                        $("#typeId2").html('<option value="' + video.id + '">' + video.title + '</option>');
