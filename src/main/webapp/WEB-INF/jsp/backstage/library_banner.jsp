...
<div class="layui-form" lay-filter="layuiadmin-form-useradmin" id="addForm"
    ...
    <div class="layui-form-item">
        <label class="layui-form-label myLabel">文库分类名称</label>
    ...
    <div class="layui-form-item">
        <label class="layui-form-label myLabel">文库名称</label>
        <div class="layui-input-inline">
...
<script>
    layui.config({
        ...
        table.render({
            elem: '#table',
            url: '/banner/list?type=3',
            method: "get",
            cols: [[
                {type: 'checkbox', fixed: 'left', align: 'center'},
                {field: 'id', title: 'ID'},
                {field: 'title', title: '轮播图标题', minWidth: 200},
                {field: 'typeName', title: '文库名称', minWidth: 200},
                {
        ...
        form.on('select(categoryId1)', function (data) {
            ...
            //动态加载下拉框值
            $.ajax({
                url: '/library/select?categoryId=' + value,
                type: "get",
                dataType: "json",
                success: function (result) {
                    data = result.data;
                    var html = '<option value="">请选择文库标题</option>';
                    $.each(data, function (index, value) {
        ...
        form.on('select(categoryId2)', function (data) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    data = result.data;
                    var html = '<option value="">请选择文库名称</option>';
                    $.each(data, function (index, value) {
        ...
        table.on('tool(table)', function (obj) {
            ...
            if (layEvent === 'del') {  //删除
                ...
                $.ajax({
                    url: '/library/detail?id=' + data.typeId,
                    type: "get",
                    success: function (result) {
                        var libraryCategory = result.data.libraryCategory;
                        var library = result.data.library;
                        //赋值
                        $("#typeId2").html('<option value="' + library.id + '">' + library.title + '</option>');
                        $("#categoryId2").find("option[value = '" + libraryCategory.id + "']").attr("selected", "selected");
                        form.render();
                    }
                });
